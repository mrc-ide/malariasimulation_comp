...
for(i in seq_along(eirs)){
  ...
  p <- get_parameters(list(
    ...
    rtss = TRUE,
    rtss_doses = round(c(0, 1.5 * month, 3 * month))
  )) %>%
    set_rtss_epi(
      start = 15 * year,
      end = 20 * year,
  ...
  ms <- ms_raw %>%
    mutate(prev_2_10 = n_detect_730_3650  / n_730_3650,
           clin = n_inc_clinical_0_1825 / n_0_1825,
           clin_smooth = roll_mean(clin, n = 365, align = "right", fill = NA) * 365,
           sev = n_inc_severe_0_1825 / n_0_1825,
  ...
  mlgts <- mlgts_raw$output %>%
    rename(clin = clin_inc_0_5,
           clin_smooth = clin_inc_0_5_smooth,
           sev = sev_inc_u5,
           sev_smooth = sev_inc_u5_smooth) %>%
    mutate(
...
inc_plot <- ggplot(output, aes(x = timestep, y = clin_smooth, col = model)) +
  geom_line() +
  ylim(0, 2.5) +
  ylab("Smooth annual\nclinical incidence (0-5)") + 
  theme_bw() +
  facet_wrap( ~ eir, nrow = 1)
sev_plot <- ggplot(output, aes(x = timestep, y = sev_smooth, col = model)) +
  geom_line() +
  ylim(0, 0.075) +
  ylab("Smooth annual\nsevere incidence (0-5)") + 
  theme_bw() +
  facet_wrap( ~ eir, nrow = 1)

summary_plot <- (prev_plot / inc_plot / sev_plot) + plot_layout(guide = "collect")

ggsave("plots/comparison_noSeasonality_rtss_exponentialDemog.png", summary_plot, width = 10, height =8)
