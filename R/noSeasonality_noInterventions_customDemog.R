...
ages <- round(demog$V3 * year) # top of age bracket
deathrates <- demog$V5 / 365 # age-specific death rates
...
for(i in seq_along(eirs)){
  ...
  p <- get_parameters(list(
    ...
    set_demography(
      ...
      birthrates = find_birthrates(pop, ages, deathrates)
    ) %>%
...
ggsave("plots/comparison_noSeasonality_noInterventions_customDemog.png", summary_plot, width = 10, height =8)
